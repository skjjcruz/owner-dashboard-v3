 ...
 *
 * iPhone MATCH view (NO HTML changes required):
 * - Desktop/iPad: original 2-table compare (Left/Right) with position group headers.
 * - Phone (<=520px): hides table panes via CSS and renders a Sleeper-like MATCH compare feed
 *   as div/cards appended to .layout (.mobileComparePane).
 *
 * Requirements (IDs expected in your HTML):
 *  #status, #teams, #activityList
 *  #usernameInput, #seasonInput, #statsSeasonInput, #leagueSelect, #reloadBtn
 *  #leftTitle, #rightTitle, #leftTable tbody, #rightTable tbody
 *************************************************/
...
// Draft pick years to show
const PICK_YEARS = [2026, 2027, 2028];

// Mobile breakpoint (matches your CSS)
...
function isPhoneMatchView() {
  return window.matchMedia && window.matchMedia(`(max-width: ${PHONE_MAX_WIDTH}px)`).matches;
}
    ...
    all.slice(0, 12).forEach((tx) => {
      ...
      li.innerHTML = `
        ...
        <div class="activityMeta">
          <strong>${ownerName}</strong> ${who ? `(${who}) ` : ""}${details}
        </div>
...
/* =========================
   Phone MATCH view helpers (div/cards)
========================= */
...
/* =========================
   Render compare tables (Desktop/iPad) OR phone MATCH view
========================= */
function renderCompareTables() {
  // If your HTML is missing tables, don't crash.
  ...
  // If we previously created the mobile pane, leave it (CSS will hide it on desktop)
  // or you can remove it here if you want:
  // document.querySelector(".mobileComparePane")?.remove();
    ...
    for (let i = 0; i < max; i++) {
      ...
      if (L[i]) {
        ...
        leftTBody.appendChild(
          trRow([
            ...
            { className: `player ${playerStatusClass(leftRoster, p)}`, text: p.full_name || "" },
      ...
      if (R[i]) {
        ...
        rightTBody.appendChild(
          trRow([
            { className: "pos posCell", text: p.position || "" },
            { className: `player ${playerStatusClass(rightRoster, p)}`, text: p.full_name || "" },
            { className: "yrs", text: yearsInLeague(p) },
            { className: "pts", text: pts ? format1(pts) : "" },
            { className: "gp", text: gp ? String(gp) : "" },
            { className: "avg", text: gp ? format1(avg) : "" },
...
async function loadLeagueData() {
  ...
  // Activity (requires roster maps for owner labels)
...
// Re-render on resize (so phone mode toggles cleanly)
...
// Boot
(function boot() {
  // Quick DOM sanity check (prevents silent “won’t load”)
  const required = [
    ["#status", elStatus],
    ["#teams", elTeams],
    ["#usernameInput", elUsername],
    ["#seasonInput", elSeason],
    ["#statsSeasonInput", elStatsSeason],
    ["#leagueSelect", elLeagueSelect],
    ["#reloadBtn", elReloadBtn],
    ["#leftTable tbody", leftTBody],
    ["#rightTable tbody", rightTBody],
    ["#activityList", elActivityList],
  ];
  const missing = required.filter(([, el]) => !el).map(([id]) => id);
  if (missing.length) {
    console.error("Missing required DOM elements:", missing);
    setStatus(`HTML missing IDs: ${missing.join(", ")}`);
    return;
  }
